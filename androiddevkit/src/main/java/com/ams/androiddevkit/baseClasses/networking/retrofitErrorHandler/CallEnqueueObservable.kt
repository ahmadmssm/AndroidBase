...
internal class CallEnqueueObservable<T>(private val originalCall: Call<T>): Observable<Response<T>>() {
    ...
    override fun subscribeActual(observer: Observer<in Response<T>?>) { // Since Call is a one-shot type, clone it for each new observer.
    ...
    private class CallCallback<T>(private val call: Call<*>, private val observer: Observer<in Response<T>?>) : Disposable, Callback<T> {
        @Volatile
